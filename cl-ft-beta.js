...
(() => {
  ...
  const LEFT = 80;
  const WIDTH = 900;
  const COLOR = 'rgba(250, 204, 21, 0.28)';
  ...
  Object.assign(highlight.style, {
    ...
    pointerEvents: 'none',
    opacity: '0',
    transition: 'top .18s ease, opacity .12s ease',
    zIndex: 9998
  });
  ...
  function showLine(i) {
    if (i < 0 || i >= LINE_COUNT) return hide();
    currentLine = i;
    highlight.style.top = (START_TOP + i * LINE_HEIGHT) + 'px';
  ...
  document.addEventListener('keydown', e => {
    ...
    if (e.key === 'ArrowDown') {
      e.preventDefault();
      showLine(currentLine === -1 ? 0 : currentLine + 1);
    }

    if (e.key === 'ArrowUp') {
      e.preventDefault();
      if (currentLine !== -1) showLine(currentLine - 1);
    }
  });
})();
